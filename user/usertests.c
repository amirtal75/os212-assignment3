...
{
  int fds[2];
  printf("countfree 1, pid: %d\n", getpid());
  if(pipe(fds) < 0){
    printf("pipe() failed in countfree()\n");
    exit(1);
  }
  printf("countfree 2, pid: %d\n", getpid());
  int pid = fork();
    
  printf("countfree 3, pid: %d\n", getpid());
  if(pid < 0){
    printf("fork failed in countfree()\n");
    exit(1);
  }
  printf("countfree 4, pid: %d\n", getpid());
  if(pid == 0){
    close(fds[0]);
    printf("countfree 5, pid: %d\n", getpid());
    while(1){
      uint64 a = (uint64) sbrk(4096);
      if(a == 0xffffffffffffffff){
        break;
      }
      printf("countfree 7, pid: %d\n", getpid());
    ...
    printf("before child exit, pid: %d\n", getpid());
    exit(0);
    printf("after child exit, pid: %d\n", getpid());
  }
  printf("countfree 6, pid: %d\n");
  close(fds[1]);
